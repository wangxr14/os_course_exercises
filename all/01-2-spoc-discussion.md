...
- 请总结你认为操作系统应该具有的特征有什么？并对其特征进行简要阐述。
  ①虚拟性：指通过交替运行，使每一个用户感觉到整个计算机都是由它一个用户在专门提供服务。
  ②并发性：在操作系统当中有多个正在运行的应用程序，我们则需要操作系统进行管理和调度，具体每个应用执行到了什么位置。
  ③异步性：只要用户的输入是一致的，那么输出结果就应该是不变。
  ④共享性：当多个应用并发运行时，宏观上要体现出它们同时在访问资源的情况，微观上又要实现它们的互斥访问。

- 为什么现在的操作系统基本上用C语言来实现？为什么没有人用python，java来实现操作系统？
  因为C语言比较底层，能比较好地描述硬件实现细节。而python、Java则不够底层。
---
